...
Thirty-six hands-on Power BI labs across six industries, each series loading
...
Six industries, same six-lab arc, different ingestion technology, because
that's the part that actually changes between a spreadsheet-driven team
and one with a real database behind it.

## How this relates to the Excel-sourced series

This is the SQL Server track of the five scenarios covered in
[powerbi (Excel edition)](https://github.com/anhubsch/powerbi): coffee
shop, university, healthcare, libraries, retail. Lab 01 in every one of
those five series loads the same underlying dataset into SQL Server through
SSMS instead of Excel and Power Query, using a staging-table-then-promote
...
differ. The sixth series, criminal justice, is specific to this repo and
has no Excel-sourced counterpart in the sibling collection.
...
| [Criminal justice](labs/criminal-justice/) | Community Payback Orders by Scottish local authority, Justice Social Work Statistics | OGL v3.0 (Scottish Government) | Real, aggregate local-authority-level statistics, with a simulated bulletin revision from Lab 06 |
...
- Internet access is needed only to download real source data: the
  coffee shop, healthcare (NHS benchmark), libraries, retail, and
  criminal justice series each link the exact file to fetch in their own
  Lab 01. The university series generates its data locally in T-SQL and
  needs no download at all.
- Three labs use Tabular Editor (free, external tool) for calculation
  groups: the healthcare, retail, and criminal justice Lab 06 capstones.
  Everything else uses only SSMS and Power BI Desktop.
...
labs/
  ...
  criminal-justice/         01-06, real Justice Social Work Statistics
```
...
- No clinical or diagnosis data anywhere in the healthcare series. It's
  ...
  in Lab 06.
- No case-level or individual-level data anywhere in the criminal justice
  series. Every figure is a local authority's aggregate count or rate for
  a whole fiscal year, exactly the grain the Scottish Government itself
  publishes at. Gender/age and gender/ethnicity crosstabs exist in the
  source only at Scotland level, with no local authority breakdown, and
  this series doesn't attempt to reconstruct one. Stated explicitly in
  that series' own Lab 01.
- No real person's data anywhere. The university series is synthetic by
  ...
  online retail transactions, criminal justice social work statistics)
  are published open data at an aggregate or transaction level, not
  records about identifiable individuals.
